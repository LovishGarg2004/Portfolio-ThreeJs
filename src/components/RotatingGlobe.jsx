...
const RotatingGlobe = () => {
    ...
    const resetTimeoutRef = useRef(null); // Store timeout ID
    ...
    useEffect(() => {
        ...
        const rotateGlobe = () => {
            if (autoRotate && globe) {
                ...
                globe.pointOfView({ ...pov, lng: pov.lng + 0.6 }); // Rotate along Y-axis (longitude)
            }
        };

        const intervalId = setInterval(rotateGlobe, 50); // Rotate every 50ms
    ...
    const handleMouseLeave = () => {
        ...
        if (!isDragging) {
            return; // If not dragging, do nothing
        }
    };
